...
import os
import gradio as gr
from crewai_tools import PDFSearchTool, WebsiteSearchTool, YoutubeVideoSearchTool
...
def setup_tools():
    ...
    yt_tool = YoutubeVideoSearchTool(
        youtube_video_url="https://www.youtube.com/watch?v=qIw-yFC-HNU",
        config=dict(
            llm=dict(
                provider="groq",
                config=dict(
    ...
    return pdf_tool, web_tool, yt_tool, web_search_tool
...
def router_tool(question):
    ...
    yt_keywords = ['Youtube', 'Basics of Investing', 'Mutual Funds', 'Bonds', 'Video']
    ...
    elif any(keyword.lower() in question.lower() for keyword in yt_keywords):
...
def create_agents():
    Router_Agent = Agent(
        role='Router',
        goal='Route user question to a pdf_vectorstore or webpage_vectorstore or youtube_vectorstore or image_analysis or websearch',
        backstory=(
            "You are an expert at routing a user question to a pdf_vectorstore or webpage_vectorstore or youtube_vectorstore or imaage_analysis or websearch. "
            "Use the pdf_vectorstore for questions related to PDF Retrieval-Augmented Generation. "
            "Use the webpage_vectorstore for questions related to Webpage Retrieval-Augmented Generation. "
            "Use the youtube_vectorstore for questions related to Youtube Retrieval-Augmented Generation. "
    ...
    YoutubeRAG_Agent = Agent(
        role="YoutubeRAG",
        goal="Use retrieved information to answer the question from Youtube video vectorstore",
        backstory=(
            "You are an assistant for question-answering tasks from Youtube video vectorstore. "
            "Provide clear, concise answers using retrieved context only from Youtube video vectorstore and nothing else."
            "Move to next Agent if question is not about Youtube video vectorstore."
        ),
        verbose=True,
        allow_delegation=False,
        llm=llm,
    ...
    return [Router_Agent, PDFRAG_Agent, WebpageRAG_Agent, YoutubeRAG_Agent, ImageAnalysis_Agent, Websearch_Agent]

# Task Definitions
def create_tasks(agents, tools):
    pdf_tool, web_tool, yt_tool, web_search_tool = tools
    Router_Agent, PDFRAG_Agent, WebpageRAG_Agent, YoutubeRAG_Agent, ImageAnalysis_Agent, Websearch_Agent = agents

    router_task = Task(
        description=(
            "Analyze the keywords in the question {question}. "
            "Decide whether it requires a pdf_vectorstore search or webpage_vectorstore search or youtube_vectorstore search or image_analysis or websearch."
        ),
        expected_output="Return 'pdf_vectorstore' or 'webpage_vectorstore' or 'youtube_vectorstore' or 'image_analysis' or 'websearch'",
        agent=Router_Agent,
        tools=[router_tool],
    )
    ...
    youtuberag_task = Task(
        description=(
            "Retrieve information for the question {question} from Youtube vectorstore only and nothing else"
            "using youtube_vectorstore based on router task."
            "Move to next Agent if question is not about Youtube vectorstore."
        ),
        expected_output="Provide retrieved information from Youtube vectorstore",
        agent=YoutubeRAG_Agent,
        context=[router_task],
        tools=[yt_tool],
    )
    ...
    return [router_task, pdfrag_task, webpagerag_task, youtuberag_task, imageanalysis_task, websearch_task]

# Main RAG Function
def run_rag_pipeline(question, history):

    # Setup tools
    pdf_tool, web_tool, yt_tool, web_search_tool = setup_tools()
    tools = (pdf_tool, web_tool, yt_tool, web_search_tool)
